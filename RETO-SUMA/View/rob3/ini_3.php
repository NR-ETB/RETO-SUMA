    ...
    if ($_FILES['csvFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if (strtolower($ext) === 'csv') {
            ...
            if (($handle = fopen($nombreTmp, 'r')) !== FALSE) {
                ...
                // Abrir o crear el archivo process_3.txt para agregar datos
                $outputFile = './process_3.txt';
                $modo = file_exists($outputFile) ? 'a' : 'w';

                if (($outputHandle = fopen($outputFile, $modo)) !== FALSE) {
                    // Leer cada línea del CSV y escribirla en process_3.txt
                    ...
                    fclose($outputHandle);
                    $mensaje = "Los datos se han almacenado correctamente en 'process_3.txt'.";
                } else {
                    $mensaje = "No se pudo abrir 'process_3.txt' para escritura.";
                }
                fclose($handle);
            } else {
