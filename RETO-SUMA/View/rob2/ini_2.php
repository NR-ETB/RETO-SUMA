    ...
    if ($_FILES['csvFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if (strtolower($ext) === 'csv') {
            ...
            if (($handle = fopen($nombreTmp, 'r')) !== FALSE) {
                ...
                // Abrir o crear el archivo process_2.csv para agregar datos
                $outputFile = './process_2.csv';
                $modo = file_exists($outputFile) ? 'a' : 'w';

                if (($outputHandle = fopen($outputFile, $modo)) !== FALSE) {
                    // Leer cada línea del CSV y escribirla en process_2.csv
                    ...
                    // $mensaje = "No se pudo abrir 'process_2.csv' para escritura.";
                }
                fclose($handle);
            } else {
                echo "<script type='text/javascript'>
